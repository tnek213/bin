    ...
    for raw in specs:
        ...
        if "*" in spec:  # pattern mode
            ...
            matches: List[str] = [
                ...
                if r.get("archived", False) and fnmatch(str(r.get("name", "")), pat)
            ]
            matches = [m for m in matches if m]
            if matches:
                messages.append(
                    f"Pattern '{owner}/{pat}' will unarchive:\n"
                    + "\n".join(f"  {m}" for m in matches)
                )
                to_unarchive.extend(matches)
            else:
                messages.append(
                    f"Pattern '{owner}/{pat}' matched 0 ARCHIVED repositories."
                )
            ...
            if not gh_ok("repo", "view", slug, "--json", "nameWithOwner"):
                invalids.append(f"{slug} (cannot view repo)")
                continue
            ...
            if archived is None:
                ...
                continue
            if not archived:
                invalids.append(f"{slug} (not archived)")
                continue
            to_unarchive.append(slug)

    to_unarchive = dedup_keep_order(to_unarchive)
    return Plan(to_unarchive, messages, needs_confirm, invalids)


def confirm(count: int, verb: str = "unarchive") -> bool:
    print()
    print(f"About to {verb} {count} repository(ies).")
    return input("Proceed? Type 'Y' to confirm: ").strip() == "Y"


def unarchive_remote(specs: List[str]) -> int:
    default_owner = config_get("DEFAULT_OWNER")
    plan = plan_remote(specs, default_owner)

    # If anything invalid, bail before doing changes
    if plan.invalids:
        print("Invalid repository targets (no changes made):")
        print("\n".join(f"  {s}" for s in plan.invalids))
        return 1

    # Confirm if patterns were used
    if plan.needs_confirm:
        print("\n".join(m for m in plan.messages if m.strip()))
        if not plan.to_unarchive:
            sys.stderr.write("Nothing to unarchive.\n")
            return 0
        if not confirm(len(plan.to_unarchive), "unarchive"):
            sys.stderr.write("Aborted by user.\n")
            return 0

    fails: List[str] = []
    for slug in plan.to_unarchive:
        # safety re-check
        archived_now = is_archived_remote(slug)
        if archived_now is False:
            sys.stderr.write(f"Not archived — skipping: {slug}\n")
            continue
        sys.stderr.write(f"Unarchiving (remote): {slug}\n")
        if not gh_ok("repo", "unarchive", slug, "-y"):
            sys.stderr.write(f"Unarchive failed: {slug}\n")
            fails.append(slug)

    if fails:
        print(f"\nFailed to unarchive {len(fails)} repository(ies):")
        print("\n".join(f"  {s}" for s in fails))
        return 1
    return 0


def unarchive_local(paths: List[str]) -> int:
    # Phase 1: validate all first (no changes if any invalid)
    invalids: List[str] = []
    plan: List[tuple[Path, str]] = []  # (path, slug)

    for p in paths:
        pp = Path(p)
        if not pp.is_dir():
            invalids.append(f"{p} (missing path)")
            continue
        if not gh_ok("repo", "view", "--json", "nameWithOwner", cwd=pp):
            invalids.append(f"{p} (not a GitHub repo)")
            continue
        slug_out = _gh(
    ...
    fails: List[str] = []
    for pp, slug in plan:
        sys.stderr.write(f"Unarchiving (local): {slug}\n")
        if not gh_ok("repo", "unarchive", "-y", cwd=pp):
            sys.stderr.write(f"Unarchive failed: {slug}\n")
            fails.append(slug)
...
"""
    ap = argparse.ArgumentParser(
        prog=PROG,
        description="Unarchive (restore) one or more GitHub repositories.",
        epilog=epilog,
    )
    ap.add_argument(
        "--remote",
    ...
    return (
        unarchive_remote(list(args.repo))
        if args.remote
        else unarchive_local(list(args.repo))
    )
